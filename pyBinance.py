...
def get_pair_candles(
    ...
    Tests:
    (1) Data for the last 6 hours, hourly candles 
    ...
    (2) Data from the last 2 months, weekly candles
    ...
    p = {'symbol': symbol, 'interval': interval, 'startTime': starttime, 'endTime': endtime, 'limit': limit}
    response = requests.get(url, params=p)
    j = response.json()

    candles = []
    for r in j:
        candle = Candle(open_time=r[0], close_time=r[6], open=float(r[1]), high=float(r[2]), low=float(r[3]), close=float(r[4]), volume=float(r[5]))
